...
public class SellerServiceImpl implements SellerService {
	...
	// 修改商家状态,进行审核
	public PygResult updateStatus(String id, TbSeller seller) {
		...
		try {
			...
			if (id.equals("2")) {
				return new PygResult(true, "审核不通过");
			} else {
				return new PygResult(true, "禁止申请,滚");
			}
		} catch (Exception e) {
			e.printStackTrace();
			return new PygResult(false, "审核修改失败");
		}
	}
